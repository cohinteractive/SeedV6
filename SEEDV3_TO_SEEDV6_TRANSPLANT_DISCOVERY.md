...
SeedV6 has a substantially better low-level chess core and perft platform, plus a small fixed-depth search skeleton. Its production path uses direct legal staged generation, PEXT-based sliding attacks, reusable board/move buffers, and `Board.makeMoveInto`. It also has a separately exercised move-type experiment. **Revision 3:** WS1–WS5 now expose a legal UCI position/search path, a managed asynchronous single-worker lifecycle with depth, node, time, clock, infinite, cancellation, replacement and shutdown control, and a V6-native rich phase-aware static evaluator. Playing strength still stops at `search/flat/FlatNegamax`, which remains full-width fixed-depth negamax rather than the complete selective search planned by later workstreams.

...
- **SEE/qsearch experimental gates (foundation resolved in Revision 4).** WS6 now proves the standalone numeric exchange contract for checks, dynamic pins, kings, promotions and en passant. Whether and where a negative/threshold SEE result may safely prune qsearch remains a separate WS9 policy question.
...
- **Revision 6:** WS1–WS8 now provide exact-search/oracle, draw/history, UCI/process, fake-clock, node-limit, lifecycle-race, rich-evaluation, SEE, TT and deterministic ordering/heuristic coverage. Qsearch and later selective-search tests remain future workstream dependencies.

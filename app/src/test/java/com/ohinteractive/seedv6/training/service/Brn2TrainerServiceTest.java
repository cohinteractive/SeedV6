...
class Brn2TrainerServiceTest {
    ...
    static TrainerConfig config(Path root, long generations) {
        return new TrainerConfig(root, 71,
                ...
                generations, TrainerConfig.DepthChange.REQUIRE_SAME, MATE, TrainingArchitecture.BRN2, .001, TrainingSource.SELF_PLAY);
    }
    ...
    @Test void realTwoGenerationSmokeBootstrapsDeterministicallyTrainsPublishesValidatesAndRecordsHistory() throws Exception {
        ...
        try (var store = new CheckpointStore(root, TrainingArchitecture.BRN2)) {
            ...
            assertEquals(TrainingArchitecture.BRN2, latest.manifest().architecture());
            assertArrayEquals(Brn2Codec.encodeTraining(new Brn2Trainer(.001)), state(root, best.manifest().id()));
            ...
            assertInstanceOf(NetworkModel.Brn2.class, latest.model());
        ...
        System.out.println("BRN2_REAL_SMOKE generations=2 selfPlayCompleted=4 aborted=0 samples=4 updates=4 validPairs=4 retained=2");
    }
    ...
    @Test void splitResumeIsBitExactAcrossServiceSerializationAndUsesStoredLearningRate() throws Exception {
        ...
        try (var service = TrainerService.fresh(config(continuous, 2), new Brn2Trainer(.001))) { a = finish(service); }
        var initial = new Brn2Trainer(.001);
        ...
        var changedInitialRate = new TrainerConfig(split, old.masterSeed(), old.selfPlay(), old.training(), old.validation(),
                1, old.depthChange(), MATE, TrainingArchitecture.BRN2, .9);
    ...
    @Test void pendingPersistedCandidateIsValidatedBeforeTrainingContinues() throws Exception {
        String candidate;
        try (var store = new CheckpointStore(root, TrainingArchitecture.BRN2)) {
            var trainer = new NetworkTrainingState.Brn2(new Brn2Trainer(.002));
        ...
        var work = new TrainerService.Operations() {
            @Override SelfPlayBatch generate(NetworkModel actor, SelfPlayConfig cfg, long[] board, SelfPlayControl control,
                ...
                assertInstanceOf(NetworkModel.Brn2.class, actor);
    ...
    @Test void brnPromotionUsesPersistedPinnedActorsAndNextGenerationUsesLatestTraining() throws Exception {
        var work = new TrainerService.Operations() {
            ...
            @Override SelfPlayBatch generate(NetworkModel actor, SelfPlayConfig cfg, long[] board, SelfPlayControl control,
                ...
                assertInstanceOf(NetworkModel.Brn2.class, actor);
                try {
                    var latest = CheckpointStore.readSnapshot(root, CheckpointInspection.reference(root, "latest-training"));
                    assertArrayEquals(Brn2Codec.encodeModel(((NetworkModel.Brn2) latest.model()).model()),
                            Brn2Codec.encodeModel(((NetworkModel.Brn2) actor).model()));
                } catch (IOException e) { throw new AssertionError(e); }
                var batch = super.generate(actor, cfg, board, control, observer);
                assertSame(actor, batch.generationModel());
                return batch;
            }
            @Override ValidationResult validate(NetworkModel candidate, NetworkModel incumbent, ValidationConfig cfg,
                ...
                assertInstanceOf(NetworkModel.Brn2.class, candidate); assertInstanceOf(NetworkModel.Brn2.class, incumbent);
                try {
                    ...
                    assertArrayEquals(Brn2Codec.encodeModel(((NetworkModel.Brn2) c.model()).model()),
                            Brn2Codec.encodeModel(((NetworkModel.Brn2) candidate).model()));
                    assertArrayEquals(Brn2Codec.encodeModel(((NetworkModel.Brn2) b.model()).model()),
                            Brn2Codec.encodeModel(((NetworkModel.Brn2) incumbent).model()));
                } catch (IOException e) { throw new AssertionError(e); }
                return TrainerServiceTest.outcome(cfg, board, history, validations++ == 0);
            }
        };
        TrainerSnapshot end;
        try (var service = TrainerService.fresh(config(root, 2), new NetworkTrainingState.Brn2(new Brn2Trainer(.001)), work, ignored -> {})) {
            end = finish(service);
        }
    ...
    @Test void stoppingAfterAnActualOptimizerUpdateDiscardsPartialWorkAndResumeReplaysExactly() throws Exception {
        Path continuous = root.resolve("continuous"), split = root.resolve("split");
        TrainerSnapshot expected;
        try (var service = TrainerService.fresh(config(continuous, 1), new Brn2Trainer(.001))) { expected = finish(service); }
        var owned = new AtomicReference<TrainerService>();
        var stopAfterUpdate = new TrainerService.Operations() {
            @Override Optional<SelfPlayTraining.Statistics> train(NetworkTrainingState trainer, SelfPlayBatch batch,
                ...
                return super.train(trainer, batch, cfg, control, progress -> {
                    ...
                    if (progress.optimizerUpdates() == 1) owned.get().stop();
                });
            }
        };
        TrainerSnapshot stopped;
        try (var service = TrainerService.fresh(config(split, 1), new NetworkTrainingState.Brn2(new Brn2Trainer(.001)),
                stopAfterUpdate, ignored -> {})) {
            owned.set(service); stopped = finish(service);
        }
        assertEquals(1, stopped.optimizerStep()); assertEquals("", stopped.candidateId());
        assertArrayEquals(Brn2Codec.encodeTraining(new Brn2Trainer(.001)), state(split, stopped.latestTrainingId()));
        TrainerSnapshot resumed;
        try (var service = TrainerService.resume(config(split, 1))) { resumed = finish(service); }

...
class Brn1ValidationWiringTest {
    @Test void realArenaUsesDistinctPinnedBrnValuesForBothColoursAndNeverFallsBack() {
        double[] weights = new double[Brn1Model.PARAMETER_COUNT]; weights[Brn1Model.OUTPUT_BIAS] = .01;
        var candidate = new NetworkModel.Brn1(new Brn1Model(weights));
        var incumbent = new NetworkModel.Brn1(new Brn1Model(new double[Brn1Model.PARAMETER_COUNT]));

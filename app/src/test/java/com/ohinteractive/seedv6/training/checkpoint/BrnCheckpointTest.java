...
class BrnCheckpointTest {
    ...
    @Test void modelAndOptimizerRoundTripAndWrongArchitectureCannotRepairOrReplaceStore() throws Exception {
        ...
        var trainer = new NetworkTrainingState.Brn(new BrnTrainer(.004));
        ...
        try (var store = new CheckpointStore(root, TrainingArchitecture.BRN)) {
            ...
            assertEquals(BrnCodec.MODEL_BYTES, manifest.networkBytes());
            assertEquals(BrnCodec.TRAINING_BYTES, manifest.trainingBytes());
            assertEquals("network.brn", manifest.networkFile());
            var restored = (NetworkTrainingState.Brn) store.resumeState(candidate);
    ...
    @Test void legacyNnueStoreResumesUnchangedAndRejectsBrnInBothDirections() throws Exception {
        ...
        var failure = assertThrows(IOException.class, () -> new CheckpointStore(root, TrainingArchitecture.BRN));
        ...
        try (var store = new CheckpointStore(root, TrainingArchitecture.NNUE)) {
            ...
            assertThrows(IOException.class, () -> store.publish(new NetworkTrainingState.Brn(new BrnTrainer(.001)),
    ...
    @Test void legacyNnueManifestAndIdentityRemainByteForByteCompatible() throws Exception {
        ...
        var brn = CheckpointManifest.create(metadata, 31, networkHash, trainingHash, TrainingArchitecture.BRN);
    ...
    @Test void recoveryAdvancesPublishedBrnCandidateAfterLatestReferenceCrash() throws Exception {
        ...
        try (var store = new CheckpointStore(root, TrainingArchitecture.BRN)) {
            ...
            assertEquals(1, latest.generation()); assertEquals(TrainingArchitecture.BRN, latest.architecture());
    ...
    @Test void corruptedBrnPayloadIsRejectedAndPreserved() throws Exception {
        String id;
        try (var store = new CheckpointStore(root, TrainingArchitecture.BRN)) {
            id = store.initialize(new NetworkTrainingState.Brn(new BrnTrainer(.001)),
                    new CheckpointManifest.Metadata(0, 1, "")).manifest().id();
        }
        Path file = root.resolve("checkpoints").resolve(id).resolve("network.brn");
    ...
    @Test void existingRetentionUsesBrnPayloadNamesAndPreservesExactLatestState() throws Exception {
        try (var store = new CheckpointStore(root, TrainingArchitecture.BRN)) {
            ...
            assertFalse(Files.exists(oldDirectory.resolve("network.brn")));
            ...
            assertFalse(inspected.materialized()); assertEquals(TrainingArchitecture.BRN, inspected.manifest().architecture());

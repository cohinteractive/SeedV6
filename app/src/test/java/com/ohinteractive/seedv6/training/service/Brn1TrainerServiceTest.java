...
class Brn1TrainerServiceTest {
    ...
    static TrainerConfig config(Path root, long generations) {
        return new TrainerConfig(root, 71,
                ...
                generations, TrainerConfig.DepthChange.REQUIRE_SAME, MATE, TrainingArchitecture.BRN1, .001, TrainingSource.SELF_PLAY);
    }
    ...
    @Test void realTwoGenerationSmokeBootstrapsDeterministicallyTrainsPublishesValidatesAndRecordsHistory() throws Exception {
        TrainerSnapshot end;
        try (var service = TrainerService.fresh(config(root, 2), new Brn1Trainer(.001))) { end = finish(service); }
        ...
        try (var store = new CheckpointStore(root, TrainingArchitecture.BRN1)) {
            ...
            assertEquals(TrainingArchitecture.BRN1, latest.manifest().architecture());
            assertArrayEquals(Brn1Codec.encodeTraining(new Brn1Trainer(.001)), state(root, best.manifest().id()));
            ...
            assertInstanceOf(NetworkModel.Brn1.class, latest.model());
        ...
        System.out.println("BRN1_REAL_SMOKE generations=2 selfPlayCompleted=4 aborted=0 samples=4 updates=4 validPairs=4 retained=2");
    }
    ...
    @Test void splitResumeIsBitExactAcrossServiceSerializationAndUsesStoredLearningRate() throws Exception {
        ...
        try (var service = TrainerService.fresh(config(continuous, 2), new Brn1Trainer(.001))) { a = finish(service); }
        var initial = new Brn1Trainer(.001);
        ...
        var changedInitialRate = new TrainerConfig(split, old.masterSeed(), old.selfPlay(), old.training(), old.validation(),
                1, old.depthChange(), MATE, TrainingArchitecture.BRN1, .9);
    ...
    @Test void pendingPersistedCandidateIsValidatedBeforeTrainingContinues() throws Exception {
        String candidate;
        try (var store = new CheckpointStore(root, TrainingArchitecture.BRN1)) {
            var trainer = new NetworkTrainingState.Brn1(new Brn1Trainer(.002));
        ...
        var work = new TrainerService.Operations() {
            @Override SelfPlayBatch generate(NetworkModel actor, SelfPlayConfig cfg, long[] board, SelfPlayControl control,
                ...
                assertInstanceOf(NetworkModel.Brn1.class, actor);
        ...
        var work = new TrainerService.Operations() {
            ...
            @Override SelfPlayBatch generate(NetworkModel actor, SelfPlayConfig cfg, long[] board, SelfPlayControl control,
                ...
                try {
                    ...
                    assertArrayEquals(Brn1Codec.encodeModel(((NetworkModel.Brn1) latest.model()).model()),
                            Brn1Codec.encodeModel(((NetworkModel.Brn1) actor).model()));
                } catch (IOException e) { throw new AssertionError(e); }
            ...
            @Override ValidationResult validate(NetworkModel candidate, NetworkModel incumbent, ValidationConfig cfg,
                ...
                assertInstanceOf(NetworkModel.Brn1.class, candidate); assertInstanceOf(NetworkModel.Brn1.class, incumbent);
                try {
                    ...
                    assertArrayEquals(Brn1Codec.encodeModel(((NetworkModel.Brn1) c.model()).model()),
                            Brn1Codec.encodeModel(((NetworkModel.Brn1) candidate).model()));
                    assertArrayEquals(Brn1Codec.encodeModel(((NetworkModel.Brn1) b.model()).model()),
                            Brn1Codec.encodeModel(((NetworkModel.Brn1) incumbent).model()));
                } catch (IOException e) { throw new AssertionError(e); }
                return TrainerServiceTest.outcome(cfg, board, history, validations++ == 0);
            }
        };
        TrainerSnapshot end;
        try (var service = TrainerService.fresh(config(root, 2), new NetworkTrainingState.Brn1(new Brn1Trainer(.001)), work, ignored -> {})) {
            end = finish(service);
        }
    ...
    void cleanStopAndResumePreserveTheLastDurableBrnState(TrainerSnapshot.State stopAt) throws Exception {
        ...
        try (var service = TrainerService.fresh(config(root, 1), new NetworkTrainingState.Brn1(new Brn1Trainer(.001)),

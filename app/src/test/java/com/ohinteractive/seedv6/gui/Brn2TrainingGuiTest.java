...
class Brn2TrainingGuiTest {
    @TempDir Path root;
    TrainingSettings settings() {
        return new TrainingSettings(root, 1, 1, 1, 0, 0, 1, 32, 1, 1, 73, 1, 1, NetworkArchitecture.BRN2, .001, .001, .002);
    }

    @Test void acceptedBrn1PreferenceKeysAndRateSurviveBrn2Selection() throws Exception {
        var prefs = Preferences.userRoot().node("seedv6-brn2-compat-" + UUID.randomUUID());
        try {
            prefs.put("architecture", "BRN1"); prefs.putDouble("brn1LearningRate", .004);
            prefs.putDouble("brnLearningRate", .007);
            var old = TrainingSettings.load(prefs);
            assertEquals(NetworkArchitecture.BRN1, old.architecture());
            assertEquals(.004, old.config(TrainerConfig.DepthChange.REQUIRE_SAME).brnLearningRate());
            assertEquals(.001, old.brn2LearningRate());
            old.save(prefs);
            assertNull(prefs.get("brn2LearningRate", null));
            prefs.put("architecture", "BRN2");
            var fresh = TrainingSettings.load(prefs); fresh.save(prefs);
            assertEquals(.001, fresh.config(TrainerConfig.DepthChange.REQUIRE_SAME).brnLearningRate());
            assertEquals(.004, prefs.getDouble("brn1LearningRate", 0));
            assertEquals(.007, prefs.getDouble("brnLearningRate", 0));
            prefs.put("architecture", "BRN1");
            assertEquals(.004, TrainingSettings.load(prefs).config(TrainerConfig.DepthChange.REQUIRE_SAME).brnLearningRate());
        } finally { prefs.removeNode(); }
    }

    @Test void legacyBrnPreferenceStaysBrn0AndBrn2DefaultsAndRateAreIndependent() throws Exception {
        var prefs = Preferences.userRoot().node("seedv6-brn2-test-" + UUID.randomUUID());
        try {
            ...
            assertEquals(.007, old.brnLearningRate()); assertEquals(.001, old.brn2LearningRate());
    ...
    @Test void preferencesRestoreBrnAndKeepNnueSettingsIndependent() throws Exception {
        ...
        try {
            ...
            assertEquals(TrainingArchitecture.BRN2, config.architecture()); assertEquals(.002, config.brnLearningRate());
    ...
    @Test void productionControllerBootstrapsAndResumesBrnWithoutNnueFallback() throws Exception {
        ...
        try {
            ...
            try (var store = new CheckpointStore(root, TrainingArchitecture.BRN2)) {
                var restored = (NetworkTrainingState.Brn2) store.resumeState(first.snapshot().latestTrainingId());
    ...
    @Test void nativeButtonsRunRealBrn2TrainingStopRestartAndResume() throws Exception {
        Assumptions.assumeFalse(GraphicsEnvironment.isHeadless());
        edt(SeedTheme::initialize);
        var options = new TrainingSettings(root, 1, 1, 2, 0, 0, 2, 32, 1, 2, 73, 4, 0,
                NetworkArchitecture.BRN2, .001, .001, .002);
        ...
        var backend = new TrainingController.Backend() {
            @Override TrainingController.Handle create(TrainingSettings settings, boolean resume, TrainerConfig.DepthChange change)
                ...
                assertEquals(TrainingArchitecture.BRN2, c.architecture());
                ...
                return handle(resume ? com.ohinteractive.seedv6.training.service.TrainerService.resume(fixture)
                        : com.ohinteractive.seedv6.training.service.TrainerService.fresh(fixture,
                                new com.ohinteractive.seedv6.core.brn2.Brn2Trainer(c.brnLearningRate())));
            }
        };
        long stoppedStep = 0;
        for (int run = 0; run < 2; run++) {
            ...
            var frame = edt(() -> {
                panel.bind(controller);
                var window = new JFrame("BRN-2 native lifecycle smoke"); window.setContentPane(panel);
            ...
            try {
                ...
                Path output = Path.of("build/gui-smoke/brn2-lifecycle-" + run + ".png"); Files.createDirectories(output.getParent());
                ImageIO.write(capture, "png", output.toFile());
            } finally { edt(controller::beginShutdown).run(); edt(frame::dispose); }
        }
        System.out.println("BRN2_NATIVE_GUI real generations/training/validation/history, Stop, controller restart, Resume passed");
    }
    ...
    @Test void nativeConfigurationCardRendersAtSmallWindowSize() throws Exception {
        ...
        try {
            ...
            Path output = Path.of("build/gui-smoke/brn2-configuration.png"); Files.createDirectories(output.getParent());

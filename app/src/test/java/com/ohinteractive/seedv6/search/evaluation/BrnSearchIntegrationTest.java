...
class BrnSearchIntegrationTest {
    ...
    @Test void evaluatorUsesRawSideToMoveValueWithPrivateScratchAndPinnedModel() {
        double[] weights = new double[BrnFeatureSchema.PARAMETER_COUNT];
        weights[BrnFeatureSchema.BIAS] = 0.25;
        BrnModel model = new BrnModel(weights);
        SearchEvaluation definition = SearchEvaluation.brn(model);
        ...
        weights[0] = -10; assertEquals(expected, a.evaluate(white, 0));
        ...
        assertThrows(IllegalArgumentException.class, () -> a.initializeFrom(white, 0, SearchEvaluation.brn(model).newState(4)));
    }
}

...
import com.ohinteractive.seedv6.core.GenLegalPext;
import com.ohinteractive.seedv6.core.util.Piece;

public final class PerftPext {
    ...
    public static void runDebug(long[] board, int maxDepth) {
        ...
        final long checkers = Board.getCheckersPext(board0, board1, board2, board3, colorMask, player, kingSquare, allOccupancy);
        int n = checkers != 0L ?
        GenLegalPext.genEvasion(
    ...
    static long perftFlat(long[] board, int depth, Workspace workspace) {
        ...
        while(top > 0) {
            ...
            // move legality check
            if(Board.isPlayerInCheckPext(nextBoard[0], nextBoard[1], nextBoard[2], nextBoard[3], frame.status & Board.PLAYER_BIT)) continue;
            if(frame.depth == 2) {
                /*
    ...
    private static long perftFlatConcurrentRoot(long[] board, int depth, int threads) {
        ...
        */
        ...
        if(checkers != 0L) {
            rootMoveCount = GenLegalPext.genEvasion(board0, board1, board2, board3, status, key, true, checkers, rootMoves, rootScratch);
        } else {
            final int tacticalCount = GenLegalPext.genTactical(board0, board1, board2, board3, status, key, true, rootMoves, rootScratch);
            final int quietCount = GenLegalPext.genQuiet(board0, board1, board2, board3, status, key, true, rootQuietMoves, rootScratch);
    ...
    private static void pushFrame(Frame frame, long[] board, int ply, int depth, Workspace workspace) {
        ...
        final long checkers = Board.getCheckersPext(board0, board1, board2, board3, colorMask, player, kingSquare, allOccupancy);
        if(checkers != 0L) {
            frame.moveCount = GenLegalPext.genEvasion(
                board0,
                board1,
                board2,
                board3,
           ...
           final int tacticalCount = GenLegalPext.genTactical(board0, board1, board2, board3, status, key, false, moves, workspace.genScratch);
           final int quietCount = GenLegalPext.genQuiet(board0, board1, board2, board3, status, key, false, quietMoves, workspace.genScratch);
    ...
    private static int countLegalMoves(long[] board, long[] moves, long[] quietMoves, long[] genScratch) {
        ...
        final long checkers = Board.getCheckersPext(board0, board1, board2, board3, colorMask, player, kingSquare, allOccupancy);
        if(checkers != 0L) {
            return GenLegalPext.genEvasion(
                board0,
                board1,
                board2,
                board3,
        ...
        */
       final int tacticalCount = GenLegalPext.genTactical(board0, board1, board2, board3, status, key, true, moves, genScratch);
       final int quietCount = GenLegalPext.genQuiet(board0, board1, board2, board3, status, key, true, quietMoves, genScratch);
    ...
    public static void divide(long[] board, int depth) {
        ...
        final long key = board[Board.KEY];
        long[] moves = moveStack[0];
        int n = GenLegalPext.genAll(board0, board1, board2, board3, status, key, true, moves, genScratch);
        long total = 0;
        long[] nextBoard = boardStack[1];
        for(int i = 0; i < n; i ++) {
            Board.makeMoveInto(board0, board1, board2, board3, status, key, moves[i], nextBoard);

...
public final class Brn2SupervisionAblation {
    ...
    enum Arm {
        ...
        double target(double wdl, double teacher) {
            return switch (this) { case WDL -> wdl; case BLENDED -> .5 * wdl + .5 * teacher; case TEACHER -> teacher; };
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        if (args.length != 3) throw new IllegalArgumentException("Expected SOURCE_STORE EXACT_NNUE_CHECKPOINT NEW_EXPERIMENT_DIRECTORY");
        ...
        try (var out = new PrintWriter(Files.newBufferedWriter(output.resolve("replay.jsonl"), StandardOpenOption.CREATE_NEW))) {
            write(out, "type", "inputs", "source", source.toString(), "g0", g0.id(), "g0ModelSha256", G0_MODEL_HASH,
                    ...
                    "target", "STM native bounded static value; blended = 0.5*WDL + 0.5*teacher; half squared error");
            // No teacher-target training can execute unless all 128 exact control comparisons pass.
    ...
    private static void runArm(Arm arm, List<Batch> batches, byte[] initialModel, byte[] initialTraining,
        ...
        for (int generation = 1; generation <= 128; generation++) {
            ...
            write(out, "type", "generation", "arm", arm, "generation", generation,
            ...
            if (MILESTONES.contains(generation)) {
                save(armPath, generation, best, out, arm);
        ...
        // Common union of the original held-out partitions: descriptive, never used for promotion.
        double[] sums = new double[3]; int count = 0;
        var workspace = new Brn2Workspace();
        for (Batch batch : batches) for (Sample sample : batch.data().partition().heldOut()) {
            double prediction = bestModel.model().evaluate(sample.board(), workspace), teacher = teacherValue(nnue, sample);
            for (Arm objective : Arm.values()) {
                double difference = prediction - objective.target(sample.target(), teacher);
                sums[objective.ordinal()] += .5 * difference * difference;
            }
            count++;
        }
        write(out, "type", "final", "arm", arm, "bestGeneration", best.generation(), "promotions", promotions,
                "modelSha256", hash(best.model()), "trainingSha256", hash(best.training()), "heldOutSamples", count,
    ...
    private static void save(Path armPath, int boundary, Snapshot best, PrintWriter out, Arm arm) throws Exception {
        ...
        var metadata = fields("arm", arm, "boundary", boundary, "bestGeneration", best.generation(),

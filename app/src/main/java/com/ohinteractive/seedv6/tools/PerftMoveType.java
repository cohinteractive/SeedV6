...
public final class PerftMoveType {

    public static void main(String[] args) {
        runAll();
        //runRange(0, 19);
        //runDebug(BoardMoveType.fromFen(POSITION_FENS[1]), 5);
    }
    ...
    public static long runSinglePosition(String fen, int depth, long expectedNodes) {
        ...
        long[] board = BoardMoveType.fromFen(fen);
    ...
    public static void runDebug(long[] board, int maxDepth) {
        ...
        long[][] boardStack = new long[maxDepth + 1][BoardMoveType.MAX_BITBOARDS];
        long[][] moveStack = new long[maxDepth + 1][MAX_MOVES];
        long[] genScratch = new long[BoardMoveType.MAX_BITBOARDS];
        ...
        for(int i = 0; i < n; i ++) {
            BoardMoveType.makeMoveInto(board0, board1, board2, board3, status, key, moves[i], nextBoard);
    ...
    public static long perftRecursiveEntry1(long[] board, int depth) {
        if(depth < 0 || depth > MAX_DEPTH) throw new IllegalArgumentException("depth");
        long[][] boardStack = new long[depth + 1][BoardMoveType.MAX_BITBOARDS];
    ...
    public static void divide(long[] board, int depth) {
        ...
        final long key = board[BoardMoveType.KEY];
        long[] moves = moveStack[0];
        int n = GenMoveType.genAll(board0, board1, board2, board3, status, key, true, moves, genScratch);
        long total = 0;
        long[] nextBoard = boardStack[1];
        for(int i = 0; i < n; i ++) {
            BoardMoveType.makeMoveInto(board0, board1, board2, board3, status, key, moves[i], nextBoard);
    ...
    private static String moveToString(long move) {
        int from = (int) move & BoardMoveType.SQUARE_BITS;
        int to = (int) move >>> BoardMoveType.TARGET_SQUARE_SHIFT & BoardMoveType.SQUARE_BITS;
        int promote = (int) move >>> BoardMoveType.PROMOTE_PIECE_SHIFT & BoardMoveType.PIECE_BITS;

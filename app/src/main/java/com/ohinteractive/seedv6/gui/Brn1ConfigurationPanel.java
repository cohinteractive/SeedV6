...
final class Brn1ConfigurationPanel extends JPanel {
    ...
    Brn1ConfigurationPanel(TrainingSettings settings) {
        ...
        learningRate = new JSpinner(new SpinnerNumberModel(settings.brn1LearningRate(), Double.MIN_VALUE, Double.MAX_VALUE, 0.0001));
        learningRate.setEditor(new JSpinner.NumberEditor(learningRate, "0.##########"));
        learningRate.setName("brn1LearningRate");
        learningRate.setToolTipText("Used only when bootstrapping a new BRN-1 store. Resume restores the exact stored learning rate and Adam state.");
        ...
        JTextArea explanation = text("""
                New BRN-1 stores start from deterministic randomized weights and fresh Adam state. Use a separate empty checkpoint folder for a new lineage.

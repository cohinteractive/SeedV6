...
/** BRN-0 online updates: one deterministic shuffled pass over the existing terminal W/D/L samples. */
public final class BrnSelfPlayTraining {
    public static Optional<SelfPlayTraining.Statistics> train(BrnTrainer trainer, SelfPlayBatch batch,
    ...
    public static Optional<SelfPlayTraining.Statistics> trainSamples(BrnTrainer trainer,
        ...
        if (config.epochs() != 1 || config.minibatchSize() != 1)
            throw new IllegalArgumentException("BRN-0 uses one online pass (epochs=1, minibatch=1).");
    ...
    private static Metrics metrics(BrnTrainer trainer, List<TrajectorySampler.Sample> samples, SelfPlayControl control) {
        long[] board = new long[Board.MAX_BITBOARDS];

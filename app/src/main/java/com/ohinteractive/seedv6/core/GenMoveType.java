...
public class GenMoveType {
    ...
    private static int getKingMoves(
        ...
        final long moveInfo = (long) square
            | ((long) piece << Board.START_PIECE_SHIFT)
            | kingCastlingChange(player);
        ...
        while(moveBitboard != 0L) {
            ...
            if(isKingDestinationSafe(
                ...
                moves[moveListLength ++] = moveInfo
                    ...
                    | ((long) targetSquare << Board.TARGET_SQUARE_SHIFT)
        ...
        if(checkers == 0L) {
            ...
            if(kingSide) {
                ...
                if((allOccupancy & intermediateSquares) == 0L
                    ...
                    moves[moveListLength ++] = moveInfo
                        | Move.CASTLE_BITS
                        | ((long) (square + 2) << Board.TARGET_SQUARE_SHIFT);
                }
            }
            if(queenSide) {
    ...
    private static int getQueenMoves(
        ...
        while(pieceBitboard != 0L) {
            ...
            final long moveInfo = (long) square | ((long) piece << Board.START_PIECE_SHIFT);
            ...
            while(moveBitboard != 0L) {
                ...
                moves[moveListLength ++] = moveInfo
                    | Move.QUIET_BITS
                    | ((long) bitScan(b2) << Board.TARGET_SQUARE_SHIFT);
            }
        }
        return moveListLength;
    }
    ...
    private static int getPawnMoves(
        ...
        while(pieceBitboard != 0L) {
            ...
            final int square = bitScan(b);
            final long moveInfo = (long) square | ((long) piece << Board.START_PIECE_SHIFT);
            final long moveInfoCapture = moveInfo | Move.CAPTURE_BITS;
            final long moveInfoEnPassant = moveInfo | Move.EN_PASSANT_BITS;
            final long moveInfoPawnPush = moveInfo | Move.PAWN_PUSH_BITS;
            final long moveInfoDoublePush = moveInfo | Move.PAWN_DOUBLE_PUSH_BITS;
            final long moveInfoPromotion = moveInfo | Move.PROMOTION_BITS;
            final long moveInfoCapturePromotion = moveInfo | Move.CAPTURE_PROMOTION_BITS;
            ...
            while(moveBitboard != 0L) {
                ...
                final int targetSquare = bitScan(b2);
                final int targetPiece = getTargetPiece(board0, board1, board2, board3, targetSquare);
                final long targetInfo = ((long) targetSquare << Board.TARGET_SQUARE_SHIFT)
                    | ((long) targetPiece << Board.TARGET_PIECE_SHIFT)
                    | capturedRookCastlingChange(targetPiece, targetSquare);
                if((targetSquare >>> 3) == promotionRank) {
                    moveListLength = addPromotions(
                        moves, moveListLength, moveInfoCapturePromotion | targetInfo, playerBit
                    );
                } else {
            ...
            if(epDestination != 0L) {
                ...
                if((otherPawns & capturedPawnBit) != 0L && checkResponse) {
                    ...
                    if(!isSquareAttacked(
                        ...
                        moves[moveListLength ++] = moveInfoEnPassant
                            | ((long) targetSquare << Board.TARGET_SQUARE_SHIFT);
                    }
                }
            }
            ...
            if((targetSquare >>> 3) == promotionRank) {
                if((singlePush & pinRay & responseMask) != 0L) {
                    moveListLength = addPromotions(
                        ...
                        moveInfoPromotion | ((long) targetSquare << Board.TARGET_SQUARE_SHIFT),
                        playerBit
                    );
                }
                continue;
            ...
            if((singlePush & pinRay & responseMask) != 0L) {
                moves[moveListLength ++] = moveInfoPawnPush
                    | ((long) targetSquare << Board.TARGET_SQUARE_SHIFT);
            }
            final long doublePush = PAWN_ADVANCE_DOUBLE[player][square]
                & ~allOccupancy & pinRay & responseMask;
            if(doublePush != 0L) {
                moves[moveListLength ++] = moveInfoDoublePush
                    | ((long) bitScan(doublePush) << Board.TARGET_SQUARE_SHIFT);
            }
        }
        return moveListLength;
    }
    ...
    private static int addPromotions(long[] moves, int moveListLength, long moveInfo, int playerBit) {
        moves[moveListLength ++] = moveInfo | ((long) (Piece.QUEEN | playerBit) << Board.PROMOTE_PIECE_SHIFT);
        moves[moveListLength ++] = moveInfo | ((long) (Piece.ROOK | playerBit) << Board.PROMOTE_PIECE_SHIFT);
        moves[moveListLength ++] = moveInfo | ((long) (Piece.BISHOP | playerBit) << Board.PROMOTE_PIECE_SHIFT);
        moves[moveListLength ++] = moveInfo | ((long) (Piece.KNIGHT | playerBit) << Board.PROMOTE_PIECE_SHIFT);
        return moveListLength;
    }

    private static long kingCastlingChange(int player) {
        return player == 0 ? Move.WHITE_CASTLING_CHANGE_MASK : Move.BLACK_CASTLING_CHANGE_MASK;
    }

    private static long rookCastlingChange(int square) {
        return switch(square) {
            case Board.SQUARE_A1 -> Move.WHITE_QUEENSIDE_CHANGE_MASK;
            case Board.SQUARE_H1 -> Move.WHITE_KINGSIDE_CHANGE_MASK;
            case Board.SQUARE_A8 -> Move.BLACK_QUEENSIDE_CHANGE_MASK;
            case Board.SQUARE_H8 -> Move.BLACK_KINGSIDE_CHANGE_MASK;
            default -> 0L;
        };
    }

    private static long capturedRookCastlingChange(int targetPiece, int targetSquare) {
        return (targetPiece & Piece.TYPE) == Piece.ROOK ? rookCastlingChange(targetSquare) : 0L;
    }
